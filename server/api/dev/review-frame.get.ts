...
/** The repo's h3 version skew, which the sibling POST route documents for
 *  `readBody`, is not limited to bodies: nitro 2.13 hands these routes a
 *  v1-shaped event (`event.node.req/res`) while the hoisted h3 is a v2 RC whose
 *  auto-imported helpers assume v2 shapes. `getQuery` does `new URL(req.url)`
 *  on what is only a PATH and throws "Invalid URL"; `setHeader` writes to
 *  `event.res.headers`, which does not exist. Both are reimplemented here
 *  against the node objects, which work under either shape — the `event.req`
 *  branches cover a future move to real v2 events.
 *
...
//
// A character-completion verdict asks a human "which two characters are in this
...
// Ported from 2xko-replay-database/server/api/dev/fuse-frame.get.ts, including
// its strict-shape query guards, which double as path-traversal guards.
//
// The character name occupies ~2% of the frame's height in the two top corners,
// which is unreadable in a thumbnail. `?band=1` returns just that strip, so a
// column of bands lets a reviewer read every sampled moment of a VOD at a
// glance — including a character CHANGING mid-set, which a single frame hides.
// Same on-the-fly-crop shape as 2XKO's fuse-hud/fuse-pill endpoints.
const BAND_FRACTION = 0.062;
...
export default defineEventHandler(async (event) => {
  ...
  return img
    .extract({ left: 0, top: 0, width, height: Math.round(height * BAND_FRACTION) })
    .png()
    .toBuffer();
});
