 ...
 * a backfill that silently covers 1,050 of 1,065 looks exactly like one that
 * worked.
 *
...
const EXPECT = { scoped: 1065, fromTag: 1065, untagged: 0 };

type Rec = Record<string, unknown> & { id: string; title: string; event?: string };

// sf6's MatchVideo carries no `intake`: it has no intake/source split, so the
// source token IS the dedupe key and the scope predicate.
const isTheater = (r: Rec): boolean => r.channel === 'replayTheater';
