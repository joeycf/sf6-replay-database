...
//
// Labelling verdicts land in data/overrides.json — a COMMITTED file with 193
// pre-existing entries — as uncommitted working-tree additions. They are the
// most expensive artifact this spike produces (a human watched footage for
// every one), and a single `git restore data/overrides.json` would erase them
// silently. So: after every labelling session, copy the sides-shaped entries
// into cache/evo/ground-truth.json, MERGING rather than replacing, and let the
...
//      tsx scripts/spike/snapshot-labels.ts --check    # exit 1 if any are uncaptured
...
/** Identity of a labelled side, order-insensitive: [ed, elena] and [elena, ed]
 *  are the same set of characters played, and only the SET is the claim — the
 *  stored order is presentational. */
...
// an archive. Merges per id (so a hand-edited "//" or a dupeOf on the same
// entry survives) and never touches ids the snapshot does not know about.
if (restore) {
  let put = 0;
  ...
  if (put) {
    // same escaping the POST route uses — data/overrides.json stores non-ASCII
    // escaped, and writing it literally reformats ~200 lines per save
    const serialized =
      JSON.stringify(overrides, null, 2).replace(
        /[\u0080-\uffff]/g,
        (c) => '\\u' + c.charCodeAt(0).toString(16).padStart(4, '0'),
      ) + '\n';
    await writeFile(join(DATA, 'overrides.json'), serialized, 'utf8');
  }
  console.log(
