...
// the roster → vote within the frame, then across frames.
//
...
// under cache/evo/frames/<id>/ so a re-run costs no downloads at all.
//
...
export interface Extraction {
   ...
   *  The first corpus pass stored only aggregate counts, which meant changing
   *  the fold (or fixing an alias) cost a full re-read of every frame. */
...
for (const [i, v] of work.entries()) {
  ...
  const rec: Extraction = {
    ...
    reads: stems.map((f, i) => ({
      f,
      p1: [reads.p1![i]!.id, reads.p1![i]!.dist === 99 ? -1 : reads.p1![i]!.dist],
      p2: [reads.p2![i]!.id, reads.p2![i]!.dist === 99 ? -1 : reads.p2![i]!.dist],
    })),
  };
  ...
  const fmt = (s: SideResult) =>
    `${(s.characters.join('+') || '—').padEnd(16)} ${s.confidence.toFixed(2)} (${s.read}/${s.sampled})` +
