...
// checked against the actual pixels instead of trusted.
//
...
for (const f of frames) {
  ...
  console.log(
    `  ${f.replace('.png', '')}  ${(r.id ?? '—').padEnd(9)} votes ${r.votes}/${r.of} dist ${r.dist === 99 ? '-' : r.dist}   raw: ${r.raw.map((x) => JSON.stringify(x)).join(' ')}`,
  );
}
